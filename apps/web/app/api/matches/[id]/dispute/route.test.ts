...
import { POST as createDispute } from './route';
...
describe('POST /api/matches/[id]/dispute - Integration Tests', () => {
  ...
  it('should return 404 for non-existent match', async () => {
    ...
    const request = new NextRequest('http://localhost/api/matches/non-existent/dispute', {
      method: 'POST',
      body: JSON.stringify({
        reason: 'Test dispute',
      }),
    ...
    const response = await createDispute(request, { params: Promise.resolve({ id: 'non-existent' }) });
